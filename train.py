...
def train(
    ...
    for task, (train_dataset, data_loader) in enumerate(zip(train_datasets, train_dataloaders), 1):
        for epoch in range(1, epochs_per_task + 1):
            ...
            for batch_index, (x, y) in data_stream:
                ...
                previous_task_iteration = sum(
                    [
                        epochs_per_task * len(d) // batch_size
                        for d in train_datasets[: task - 1]
                    ]
                )
                ...
                sigmoid_tensor = torch.sigmoid(scores)

                # Apply a threshold of 0.5 to convert probabilities to binary values
                predicted = (sigmoid_tensor > 0.5).float()
                # calculate the training precision.
                # _, predicted = scores > 0.5
                ...
                data_stream.set_description(
                    (
                        ...
                        total=dataset_size,
                        progress=(100.0 * batch_index / dataset_batches),
                ...
                if iteration % eval_log_interval == 0:
                    names = [
                        "task {}".format(i + 1) for i in range(len(train_datasets))
                    ]
                    precs = [
                        (
                            utils.validate(
                    ...
                    visual.visualize_scalars(vis, precs, names, title, iteration)
                ...
                if iteration % loss_log_interval == 0:
                    ...
                    visual.visualize_scalars(
                        vis,
                        [loss.cpu(), ce_loss.cpu(), ewc_loss.cpu()],
        ...
        if consolidate and task < len(train_datasets):
            ...
            model.consolidate(
                model.estimate_fisher(train_dataset, data_loader, fisher_estimation_sample_size)
            )
            print(" Done!")
